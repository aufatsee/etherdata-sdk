...
/**
 * The txpool API gives you access to several non-standard RPC methods to inspect the contents of
...
class Txpool(val url: String) {
   ...
   * arrays, in which each entry maps an origin-address to a batch of scheduled transactions These
   * batches themselves are maps associating nonces with actual transactions
   *
   * Please note, there may be multiple transactions associated with the same account and nonce This
   * can happen if the user broadcast mutliple ones with varying gas allowances (or even complerely
   * different transactions)
   * @return transactionObject The return transaction object
   */
  suspend fun content(): ContentResponseTransactionObject {

    val response: JsonRpcResponse<ContentResponseTransactionObject> =
        client.post(url) {
          contentType(ContentType.Application.Json)
          body =
              JsonRpcRequest(method = "txpool_content", params = listOf(), jsonrpc = "2.0", id = 1)
        }
   ...
   * The result is an object with two fields pending and queued Each of these fields are associative
   * arrays, in which each entry maps an origin-address to a batch of scheduled transactions These
   * batches themselves are maps associating nonces with transactions summary strings
   *
   * Please note, there may be multiple transactions associated with the same account and nonce This
   * can happen if the user broadcast mutliple ones with varying gas allowances (or even complerely
   * different transactions)
  ...
  suspend fun inspect(): InspectResponseTransactionObject {
    ...
    val response: JsonRpcResponse<InspectResponseTransactionObject> =
        client.post(url) {
          contentType(ContentType.Application.Json)
          body =
              JsonRpcRequest(method = "txpool_inspect", params = listOf(), jsonrpc = "2.0", id = 1)
        }
    return response.result
  }

  /**
   * The status inspection property can be queried for the number of transactions currently pending
  ...
  suspend fun status(): StatusResponseStatusObject {
    ...
    val response: JsonRpcResponse<StatusResponseStatusObject> =
        client.post(url) {
          contentType(ContentType.Application.Json)
          body =
              JsonRpcRequest(method = "txpool_status", params = listOf(), jsonrpc = "2.0", id = 1)
        }
    return response.result
  }
}
