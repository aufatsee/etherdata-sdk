...
class Etd(val url: String) {
  ...
  /**
   * This method is used for real-time events through subscriptions See the subscription
  ...
  suspend fun subscribe(): Void {
    ...
    val response: JsonRpcResponse<Void> =
        client.post(url) {
          ...
          body =
              JsonRpcRequest(method = "eth_subscribe", params = listOf(), jsonrpc = "2.0", id = 1)
        }
  ...
  suspend fun unsubscribe(): Void {
    ...
    val response: JsonRpcResponse<Void> =
        client.post(url) {
          contentType(ContentType.Application.Json)
          body =
              JsonRpcRequest(method = "eth_unsubscribe", params = listOf(), jsonrpc = "2.0", id = 1)
        }
    return response.result
  }
}
