...
class Personal(val url: String) {
  ...
  /**
   * Imports the given unencrypted private key (hex string) into the key store, encrypting it with
   * the passphrase Returns the address of the new account
  ...
  suspend fun listAccounts(): List<String> {
    ...
    val response: JsonRpcResponse<List<String>> =
        client.post(url) {
          ...
          body =
              JsonRpcRequest(
                  method = "personal_listAccounts", params = listOf(), jsonrpc = "2.0", id = 1)
        }
    return response.result
  }

  /**
   * Removes the private key with given address from memory The account can no longer be used to
   * send transactions
   */
  suspend fun lockAccount(): Void {
    ...
    val response: JsonRpcResponse<Void> =
        client.post(url) {
          ...
          body =
              JsonRpcRequest(
                  method = "personal_lockAccount", params = listOf(), jsonrpc = "2.0", id = 1)
        }
    return response.result
  }
   ...
   *
   * At the Getd console, newAccount will prompt for a passphrase when it is not supplied as the
   * argument
   ...
   *
   * Both passphrase and unlock duration are optional when using the JavaScript console If the
   * passphrase is not supplied as an argument, the console will prompt for the passphrase
   * interactively
   *
   * The unencrypted key will be held in memory until the unlock duration expires If the unlock
   * duration defaults to 300 seconds An explicit duration of zero seconds unlocks the key until
   * Getd exits
   *
   * The account can be used with etd_sign and etd_sendTransaction while it is unlocked
   * @param accountAddress The account address
   * @param passphrase The passphrase If you want to type in the passphrase and stil override the
   * default unlock duration, pass null as the passphrase.
   ...
   * is verified, signed and send onto the network The account is not unlocked globally in the node
   ...
   *
   * Example &#39;&gt; var tx = {from&#39;:&#39;
   * &quot;0x391694e7e0b0cce554cb130d723a9d27458f9298&quot;, to&#39;:&#39;
   * &quot;0xafa3f8684e54059998bc3a7b0d2b0da075154d66&quot;, value&#39;:&#39; web3 toWei(1 23,
   * &quot;ether&quot;)} undefined &quot;&gt; personal sendTransaction(tx,
   * &quot;passphrase&quot;)&quot;
  ...
  suspend fun sendTransaction(tx: Tx): String {
    ...
    val response: JsonRpcResponse<String> =
        client.post(url) {
          ...
          body =
              JsonRpcRequest(
                  method = "personal_sendTransaction", params = listOf(tx), jsonrpc = "2.0", id = 1)
        }
    return response.result
  }
  ...
  suspend fun sign(a: String, b: String, c: String): String {
    ...
    val response: JsonRpcResponse<String> =
        client.post(url) {
          ...
          body =
              JsonRpcRequest(
                  method = "personal_sign", params = listOf(a, b, c), jsonrpc = "2.0", id = 1)
        }
    return response.result
  }

  /**
   * ecRecover returns the address associated with the private key that was used to calculate the
  ...
  suspend fun ecRecover(a: String, b: String): String {
    ...
    val response: JsonRpcResponse<String> =
        client.post(url) {
          ...
          body =
              JsonRpcRequest(
                  method = "personal_ecRecover", params = listOf(a, b), jsonrpc = "2.0", id = 1)
        }
    return response.result
  }
}
